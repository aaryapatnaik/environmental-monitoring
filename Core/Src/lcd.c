// raw 4-bit parallel HD44780 driver, no library, just direct GPIO writes.
// see lcd.h for the pin mapping

#include "lcd.h"
#include "stm32f446xx.h"

// pin definitions, derived from the wiring table in lcd.h
...
// enables the cpu cycle counter, used for the microsecond delays below
...
static void lcd_pulse_enable(void)
{
    ...
    lcd_delay_us(1);   // en pulse width needs to be > 450ns
    gpio_write(LCD_EN_PORT, LCD_EN_PIN, 0);
...
// runs the hd44780 power-on/4-bit-mode init sequence and leaves the
// display cleared and ready to print to
void LCD_Init(void)
{
    dwt_init();
    ...
    // then switch to 4-bit mode. has to be done exactly this way, it's
    // how the controller detects it's talking to a 4-bit bus
    lcd_write4(0x03);
    lcd_delay_ms(5);
    lcd_write4(0x03);
    lcd_delay_us(150);
...
// prints str on the given row, left-justified, and pads or truncates it
// to fill all 16 columns so leftover characters from a longer previous
// line don't stick around on screen
