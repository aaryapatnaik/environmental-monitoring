...
static void gpio_out_init(GPIO_TypeDef *port, uint8_t pin)
{
    ...
    // push-pull is enough here since these lines only ever drive lcd inputs, never shared
...
// power-on delay and 4-bit handshake below are required by the hd44780 spec, not tunable
void LCD_Init(void)
{
    dwt_init();
    ...
    // exact sequence/timing here is how the controller detects a 4-bit bus, don't reorder
    lcd_write4(0x03);
    lcd_delay_ms(5);
    lcd_write4(0x03);
    lcd_delay_us(150);
...
// pads with spaces so leftover characters from a longer previous line don't stick around
